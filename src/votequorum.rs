    ...
    expected_votes: u32)
{
	...
	Some(h) =>  {
	    (h.callbacks.expectedvotes_notification_fn)(handle,
							context,
							expected_votes);
	}
	None => {}
    ...
    node_list: *mut ffi::votequorum::votequorum_node_t)
{
	...
	Some(h) =>  {
	    ...
	    (h.callbacks.quorum_notification_fn)(handle,
						 context,
						 r_quorate,
						 r_node_list);
	}
    ...
    node_list: *mut u32)
{
	...
	Some(h) =>  {
	    ...
	    (h.callbacks.nodelist_notification_fn)(handle,
						   context,
						   r_ring_id,
						   r_node_list);
	}
...
pub struct Callbacks {
    ...
    pub nodelist_notification_fn: fn(hande: u64,
				     ...
				     ring_id: RingId,
				     node_list: Vec<u32>),
    pub expectedvotes_notification_fn: fn(handle: u64,
					  context: u64,
					  expected_votes: u32),
}
...
//#[derive(Copy, Clone)]
struct Handle {
    _votequorum_handle: u64, // We *might* need this??
    callbacks: Callbacks
}
...
pub fn initialize(callbacks: &Callbacks) -> Result<u64>
{
	...
	if res == ffi::votequorum::CS_OK {
	    let rhandle = Handle{_votequorum_handle: handle, callbacks: callbacks.clone()};
...
pub fn finalize(handle: u64) -> Result<()>
{
    ...
    let res =
	unsafe {
	    ffi::votequorum::votequorum_finalize(handle)
	};
    if res == ffi::votequorum::CS_OK {
	Ok(())
    } else {
...
pub fn fd_get(handle: u64) -> Result<i32>
{
    ...
    let res =
	unsafe {
	    ffi::votequorum::votequorum_fd_get(handle, c_fd)
	};
    if res == ffi::votequorum::CS_OK {
	Ok(c_fd as i32)
    } else {
...
pub fn get_info(handle: u64, nodeid: u32) -> Result<NodeInfo>
{
    ...
    let res =
	unsafe {
	    ffi::votequorum::votequorum_getinfo(handle, nodeid, &mut c_info)
	};
...
fn string_from_bytes(bytes: &[i8;VOTEQUORUM_QDEVICE_MAX_NAME_LEN]) -> Option<String>
{
...
pub fn dispatch(handle: u64, flags: DispatchFlags) -> Result<()>
{
    let res =
	unsafe {
	    ffi::votequorum::votequorum_dispatch(handle, flags as u32)
	};
    if res == ffi::votequorum::CS_OK {
	Ok(())
    } else {
...
/// handle: a votequorum handle as returned from [initialize]
/// flags: [TrackFlags]
pub fn trackstart(handle: u64, context: u64,flags: TrackFlags) -> Result<()>
{
    let res =
	unsafe {
	    ffi::votequorum::votequorum_trackstart(handle, context, flags as u32)
	};
    if res == ffi::votequorum::CS_OK {
	Ok(())
    } else {
...
pub fn trackstop(handle: u64) -> Result<()>
{
    let res =
	unsafe {
	    ffi::votequorum::votequorum_trackstop(handle)
	};
    if res == ffi::votequorum::CS_OK {
	Ok(())
    } else {
...
pub fn context_get(handle: u64) -> Result<u64>
{
	...
	unsafe {
	    ...
	    let r = ffi::votequorum::votequorum_context_get(handle, &mut c_context);
...
pub fn context_set(handle: u64, context: u64) -> Result<()>
{
	...
	unsafe {
	    ...
	    ffi::votequorum::votequorum_context_set(handle, c_context)
	};
    if res == ffi::votequorum::CS_OK {
	Ok(())
    } else {
...
pub fn set_expected(handle: u64, expected_votes: u32) -> Result<()>
{
    let res =
	unsafe {
	    ffi::votequorum::votequorum_setexpected(handle, expected_votes)
	};
    if res == ffi::votequorum::CS_OK {
	Ok(())
    } else {
...
pub fn set_votes(handle: u64, nodeid: u32, votes: u32) -> Result<()>
{
    let res =
	unsafe {
